...
class ControlVideoPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.safety_checker = lambda images, clip_input: (images,None)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        prompt_embeds_inversion = self._encode_prompt(
            "",
            device,
            num_videos_per_prompt,
            do_classifier_free_guidance,
        ...
        if kwargs['attn_fusion_rate'] != 1.0:
            # 8. DDIM inversion
            ...
            with self.progress_bar(total=num_inverse_steps) as progress_bar:
                for i, t in enumerate(inverse_timesteps):
                    ...
                    down_block_res_samples, mid_block_res_sample = self.controlnet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=prompt_embeds_inversion,
            ...
            video = torch.from_numpy(video).squeeze().permute(1, 2, 3, 0)
        ...
        # 10. feature-based correspondence computing
        latents = latents_origin
        noise = torch.randn_like(latents)
        latents = self.scheduler.add_noise(latents, noise, torch.tensor([kwargs['diff_t']],dtype=torch.int32))
        latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
        latent_model_input = self.scheduler.scale_model_input(latent_model_input, kwargs['diff_t'])
        diff_ft = self.unet(
        latent_model_input,
        kwargs['diff_t'],
        encoder_hidden_states=prompt_embeds,
        ...
        **kwargs_tempo,
        ...
        H_list = [64, 32, 16, 8]
        diff_ft = rearrange(diff_ft[3][0], "c n h w -> n c h w")
        for q, size in enumerate(H_list):
            corr_all = []
            sim_all = []
            mask_all = []
        
            diff_ft_now = F.interpolate(diff_ft,size=(kwargs['corr'][q].size(2),kwargs['corr'][q].size(3)),mode='bilinear',align_corners=False)
        
            diff_ft_now = rearrange(diff_ft_now, "n c h w -> n (h w) c")
            with torch.no_grad():
                for i in range(diff_ft_now.size(0)):
                    corr_cur = []
                    # sim_cur = []
                    mask_cur = []
                    cur_ft = F.normalize(diff_ft_now[i], dim=1, p=2)
                    for j in range(diff_ft_now.size(0)):
                        ref_ft = F.normalize(diff_ft_now[j], dim=1, p=2)
                        cos_sim = torch.exp(torch.bmm(cur_ft.unsqueeze(0), ref_ft.permute(1, 0).unsqueeze(0))).squeeze()
                        # sim_cur.append(cos_sim.unsqueeze(0).cpu())
                        corr_value, corr = torch.max(cos_sim, 1)
        
                        mask = rearrange(corr_value > kwargs['feature_threshold'], "(h w) -> h w",
                                         h=kwargs['corr'][q].size(2), w=kwargs['corr'][q].size(3))
        
                        # mask = F.interpolate(mask.float().unsqueeze(0).unsqueeze(0), size=(kwargs['corr'][q].size(2),kwargs['corr'][q].size(3)), mode='nearest').bool().squeeze()
        
                        mask_cur.append(mask.unsqueeze(0))
                        x = corr % diff_ft.size(2)
                        y = corr // diff_ft.size(3)
                        x_norm = 2.0 * (x.float() / (diff_ft.size(2) - 1)) - 1
                        y_norm = 2.0 * (y.float() / (diff_ft.size(3) - 1)) - 1
                        corr_grid = torch.stack((x_norm, y_norm), dim=-1)
                        corr_grid = rearrange(corr_grid, "(h w) c -> h w c", h=kwargs['corr'][q].size(2),
                                              w=kwargs['corr'][q].size(3))
                        # corr_grid = F.interpolate(corr_grid.permute(2,0,1).unsqueeze(0),size=(kwargs['corr'][q].size(2),kwargs['corr'][q].size(3)),mode='bilinear',align_corners=False).squeeze(0).permute(1,2,0)
                        corr_cur.append(corr_grid.unsqueeze(0))
                        gc.collect()
                        print(i,j)
                    corr_cur = torch.cat(corr_cur, 0)
                    # sim_cur = torch.cat(sim_cur, 0)
                    mask_cur = torch.cat(mask_cur, 0)
                    corr_all.append(corr_cur.unsqueeze(0))
                    # sim_all.append(sim_cur.unsqueeze(0))
                    mask_all.append(mask_cur.unsqueeze(0))
                corr_all = torch.cat(corr_all, 0)
                mask_all = torch.cat(mask_all, 0)
                kwargs['corr'][q] = kwargs['corr'][q] * kwargs['mask_wrap'][q].unsqueeze(4) + corr_all * ~kwargs['mask_wrap'][q].unsqueeze(4)
                kwargs['mask_wrap'][q] = kwargs['mask_wrap'][q] | mask_all
            del diff_ft_now
        ...
        with self.progress_bar(total=num_inverse_steps) as progress_bar:
            for i, t in enumerate(inverse_timesteps):
                ...
                if i == len(inverse_timesteps) - 1 or (
