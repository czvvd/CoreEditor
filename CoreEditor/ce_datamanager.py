...
from CoreEditor.ce_dataset import CEDataset
...
@dataclass
class CoreEditorDataManagerConfig(FullImageDatamanagerConfig):
    """Configuration for the CoreEditorDataManager."""

    _target: Type = field(default_factory=lambda: CoreEditorDataManager)
    patch_size: int = 32
    ...
    sampled_views_every_subset: int = 5
    """Number of images sampled in each subset split"""
    load_all: bool = False
    """Set it to True if you want to edit all the images in the dataset"""


class CoreEditorDataManager(FullImageDatamanager, Generic[TDataset]):
    """Data manager for CoreEditor."""

    config: CoreEditorDataManagerConfig
    ...
    def __init__(self,
        ...
        for i in range(len(self.cached_train)):
            self.cached_train[i]['original'] = self.cached_train[i]['image']
        
        # Sample data
        if len(self.train_dataset._dataparser_outputs.image_filenames) <= self.config.subset_num or self.config.load_all:
            ...
            # original
            # anchors = list(range(0, view_num, view_num // self.config.subset_num))[:self.config.subset_num] + [view_num]
            # sampled_indices = []
            ...
            # Ours
            # self.sampled_indices = list(range(0, view_num, view_num // (self.config.subset_num-1)))+[view_num-1]
            self.sampled_indices = np.linspace(0,view_num-1,self.config.subset_num,dtype=int).tolist()

            self.cameras = [self.train_dataset.cameras[i: i+1] for i in self.sampled_indices]
            self.train_data_temp = [self.cached_train[i] for i in self.sampled_indices]
            self.train_data = []
            for i, data in enumerate(self.train_data_temp):
                data['image_idx'] = i
                self.train_data.append(data)
            self.train_unseen_cameras = list(range(self.config.subset_num))
    ...
    def update_dateset(self):
        view_num = len(self.train_dataset._dataparser_outputs.image_filenames)
        sampled_indices1 = []
        for idx in range(len(self.sampled_indices[:-1])):
            cur_anchor = self.sampled_indices[idx]
            next_anchor = self.sampled_indices[idx + 1]
            selected = sorted(
                random.sample(list(range(cur_anchor, next_anchor)), 1))
            sampled_indices1 += selected
        sampled_indices1 = sampled_indices1 + [view_num - 1]
        self.sampled_indices = sampled_indices1
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[CoreEditorDataManager] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is CoreEditorDataManager and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is CoreEditorDataManager:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is CoreEditorDataManager:
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        if len(self.train_dataset._dataparser_outputs.image_filenames) <= self.config.subset_num:
